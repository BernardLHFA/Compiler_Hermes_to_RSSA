...
struct
  ...
  fun remove_Variable x [] = []
    | remove_Variable x ((t, a, n, b) :: res) =
        if x = a
        then remove_Variable x res
        else [(t, a, n, b)]@(remove_Variable x res)
  ...
  fun init_MSP msp var p =
        ...
        then ([], var)
          ...
          let
            val (msp, var2) = get_Variable var msp (Data.TypeS(Data.Public, Data.u64)) var false
            val expr = [Data.Assign2S((Data.TypeS(Data.Public, Data.u64)), Data.VarS(msp, p), Data.CstS("0", p), p)]
          in
            (expr, var2)
          end
  ...
  fun array_Decl_Loop s e t result entry size var label incr p =
        let
          val var2 = incr_Variable var "i"
          val (i0, var3) = get_Variable var2 "i" (Data.TypeS(Data.Public, Data.u64)) var2 false
          val (start1, expr1, var4) = comp_E_condD (Hermes.Const("0", p)) "+" (Data.TypeS(Data.Public, Data.u64)) i0 "0" p p var3 true (label^(Int.toString incr)^"T1") (label^(Int.toString incr)^"I1")
          val (start2, _, var5) = comp_E_condD e "+" (Data.TypeS(Data.Public, Data.u64)) i0 "0" p p var4 true (label^(Int.toString incr)^"T2") (label^(Int.toString incr)^"I2")
          val args = get_Arguments var5 p []
          val exit1 = Data.UncondExitS((label^(Int.toString incr)^"init", p), args, p)
          val newblock = Data.BlockS(entry, result@start1@start2@expr1, exit1, p)
          ...
          val cond1 = get_Condition_Equal i1 (Hermes.Const("0", p)) var7 p (label^(Int.toString incr)^"T1")
          val entry2 = Data.CondEntryS(cond1, (label^(Int.toString incr)^"init", p), (label^(Int.toString incr)^"loop", p), args2, p)
          val (msp1, var8) = get_Variable var7 s (Data.TypeS(Data.Public, Data.u64)) var7 false
          val var9 = incr_Variable var8 s
          val (msp2, var10) = get_Variable var9 s (Data.TypeS(Data.Public, Data.u64)) var9 false
          val (i1, var11) = get_Variable var10 "i" (Data.TypeS(Data.Public, Data.u64)) var10 false
          val var12 = incr_Variable var11 "i"
          val (i2, var13) = get_Variable var12 "i" (Data.TypeS(Data.Public, Data.u64)) var12 false
          val loop = [Data.SwapS(Data.CstS("0", p), Data.MemoryS(t, Data.VarS(msp1, p), p), Data.CstS("0", p), p),
                      Data.AssignS(Data.TypeS(Data.Public, Data.u64), Data.VarS(msp2, p), Data.UpdOp2S("+", Data.VarS(msp1, p), Data.SimOp2S(Data.CstS(Int.toString(size), p)), p), p),
                      Data.AssignS(Data.TypeS(Data.Public, Data.u64), Data.VarS(i2, p), Data.UpdOp2S("+", Data.VarS(i1, p), Data.SimOp2S(Data.CstS("1", p)), p), p)]
          val args3 = get_Arguments var13 p []
          val cond2 = get_Condition_Equal i2 e var13 p (label^(Int.toString incr)^"T2")
          val exit2 = Data.CondExitS(cond2, (label^(Int.toString incr)^"done", p), (label^(Int.toString incr)^"loop", p), args3, p)
          val newblock2 = Data.BlockS(entry2, loop, exit2, p)
          val var14 = incr_Arguments var13
          val args4 = get_Arguments var14 p []
          val entry3 = Data.UncondEntryS((label^(Int.toString incr)^"done", p), args4, p)
          val (i3, var15) = get_Variable var14 "i" (Data.TypeS(Data.Public, Data.u64)) var14 false
          val (expr2, finish2, var16) = comp_E_condF e "-" (Data.TypeS(Data.Public, Data.u64)) "0" i3 p p var15 true (label^(Int.toString incr)^"T2") (label^(Int.toString incr)^"I2")
          val (_, finish1, var17) = comp_E_condF (Hermes.Const("0", p)) "-" (Data.TypeS(Data.Public, Data.u64)) "0" i3 p p var16 true (label^(Int.toString incr)^"T1") (label^(Int.toString incr)^"I1")
        in
  ...
  fun array_Undecl_Loop s e t result entry size var label incr p =
        let
          val var2 = incr_Variable var "i"
          val (i0, var3) = get_Variable var2 "i" (Data.TypeS(Data.Public, Data.u64)) var2 false
          val (start1, expr1, var4) = comp_E_condD e "+" (Data.TypeS(Data.Public, Data.u64)) i0 "0" p p var3 true (label^(Int.toString incr)^"T1") (label^(Int.toString incr)^"I1")
          val (start2, _, var5) = comp_E_condD (Hermes.Const("0", p)) "+" (Data.TypeS(Data.Public, Data.u64)) i0 "0" p p var4 true (label^(Int.toString incr)^"T2") (label^(Int.toString incr)^"I2")
          val args = get_Arguments var5 p []
          val exit1 = Data.UncondExitS((label^(Int.toString incr)^"uninit", p), args, p)
          val newblock = Data.BlockS(entry, result@start1@start2@expr1, exit1, p)
          val var6 = incr_Arguments var5
          val args2 = get_Arguments var6 p []
          val (i1, var7) = get_Variable var6 "i" (Data.TypeS(Data.Public, Data.u64)) var6 false
          val cond1 = get_Condition_Equal i1 e var7 p (label^(Int.toString incr)^"T1")
          val entry2 = Data.CondEntryS(cond1, (label^(Int.toString incr)^"uninit", p), (label^(Int.toString incr)^"unloop", p), args2, p)
          val (msp1, var8) = get_Variable var7 s (Data.TypeS(Data.Public, Data.u64)) var7 false
          val var9 = incr_Variable var8 s
          val (msp2, var10) = get_Variable var9 s (Data.TypeS(Data.Public, Data.u64)) var9 false
          val (i1, var11) = get_Variable var10 "i" (Data.TypeS(Data.Public, Data.u64)) var10 false
          val var12 = incr_Variable var11 "i"
          val (i2, var13) = get_Variable var12 "i" (Data.TypeS(Data.Public, Data.u64)) var12 false
          val loop = [Data.AssignS(Data.TypeS(Data.Public, Data.u64), Data.VarS(i2, p), Data.UpdOp2S("-", Data.VarS(i1, p), Data.SimOp2S(Data.CstS("1", p)), p), p),
                      Data.AssignS(Data.TypeS(Data.Public, Data.u64), Data.VarS(msp2, p), Data.UpdOp2S("-", Data.VarS(msp1, p), Data.SimOp2S(Data.CstS(Int.toString(size), p)), p), p),
                      Data.SwapS(Data.CstS("0", p), Data.MemoryS(t, Data.VarS(msp2, p), p), Data.CstS("0", p), p)]
          val args3 = get_Arguments var13 p []
          val cond2 = get_Condition_Equal i2 (Hermes.Const("0", p)) var13 p (label^(Int.toString incr)^"T2")
          val exit2 = Data.CondExitS(cond2, (label^(Int.toString incr)^"undone", p), (label^(Int.toString incr)^"unloop", p), args3, p)
          val newblock2 = Data.BlockS(entry2, loop, exit2, p)
          val var14 = incr_Arguments var13
          val args4 = get_Arguments var14 p []
          val entry3 = Data.UncondEntryS((label^(Int.toString incr)^"undone", p), args4, p)
          val (i3, var15) = get_Variable var14 "i" (Data.TypeS(Data.Public, Data.u64)) var14 false
          val (expr2, finish2, var16) = comp_E_condF (Hermes.Const("0", p)) "-" (Data.TypeS(Data.Public, Data.u64)) "0" i3 p p var15 true (label^(Int.toString incr)^"T2") (label^(Int.toString incr)^"I2")
          val (_, finish1, var17) = comp_E_condF e "-" (Data.TypeS(Data.Public, Data.u64)) "0" i3 p p var16 true (label^(Int.toString incr)^"T1") (label^(Int.toString incr)^"I1")
        in
          (expr2@finish2@finish1, entry3, [newblock]@[newblock2], var17)
        end
  
  (* Compile Declarations *)
  fun comp_Decl [] result entry var label incr pu s = ([], result, entry, [], var, pu, s)
    | comp_Decl (Hermes.ConstDecl(y, i, p) :: res) result entry var label incr pu s =
        let
          ...
          val (finish, result2, entry2, block, var3, pu2, s2) = comp_Decl res (result@start) entry var2 label incr pu s
        in
          (finish, result2, entry2, block, var3, pu2, s2)
        end
    | comp_Decl (Hermes.VarDecl(y, t, p) :: res) result entry var label incr pu s =
        let
          val t0 = comp_Type t
    ...
    | comp_Decl (Hermes.ArrayDecl(y, t, e, p) :: res) result entry var label i pu s =
        let
          val t0 = comp_Type t
        ...
        in
          ...
          then
            let
              val (smspFirst, var2) = init_MSP "SMSP" var p
              val (yt, var3) = get_Variable var2 (y^"T") t0 var2 true
              val (smsp, var4) = get_Variable var3 "SMSP" (Data.TypeS(Data.Public, Data.u64)) var3 false
              val start = [Data.AssignS((Data.TypeS(Data.Public, Data.u64), Data.VarS(yt, p), Data.UpdOp2S("+", Data.CstS("0", p), Data.SimOp2S(Data.VarS(smsp, p)), p), p))]
              val var5 = incr_Variable var4 (y^"S")
              val (ys, var6) = get_Variable var5 (y^"S") t0 var5 true
              val sizeShift = (get_Power (get_Size t0)) - 3
              val (expr, var7) = comp_E (Hermes.Bin(Hermes.ShiftL, e, Hermes.Const(Int.toString(sizeShift), p), p)) "^" (Data.TypeS(Data.Public, Data.u64)) ys "0" p p var6 true
              val (result2, entry2, block, var8) = array_Decl_Loop "SMSP" e t0 (result@smspFirst@start@expr) entry size var7 label i p
              val (finish, result3, entry3, block2, var9, pu2, s2) = comp_Decl res result2 entry2 var8 label (i+1) pu (s+(incr*size))
            in
              (finish, result3, entry3, block@block2, var9, pu2, s2)
            end
          else
            let
              val (pmspFirst, var2) = init_MSP "PMSP" var p
              val (yt, var3) = get_Variable var2 (y^"T") t0 var2 true
              val (pmsp, var4) = get_Variable var3 "PMSP" (Data.TypeS(Data.Public, Data.u64)) var3 false
              val start = [Data.AssignS((Data.TypeS(Data.Public, Data.u64), Data.VarS(yt, p), Data.UpdOp2S("+", Data.CstS("0", p), Data.SimOp2S(Data.VarS(pmsp, p)), p), p))]
              val var5 = incr_Variable var4 (y^"S")
              val (ys, var6) = get_Variable var5 (y^"S") t0 var5 true
              val sizeShift = (get_Power (get_Size t0)) - 3
              val (expr, var7) = comp_E (Hermes.Bin(Hermes.ShiftL, e, Hermes.Const(Int.toString(sizeShift), p), p)) "^" (Data.TypeS(Data.Public, Data.u64)) ys "0" p p var6 true
              val (result2, entry2, block, var8) = array_Decl_Loop "PMSP" e t0 (result@pmspFirst@start@expr) entry size var7 label i p
              val (finish, result3, entry3, block2, var9, pu2, s2) = comp_Decl res result2 entry2 var8 label (i+1) pu (s+(incr*size))
            in
              (finish, result3, entry3, block@block2, var9, pu2, s2)
            end
  ...
  fun comp_Undecl [] result entry var label incr pu s = ([], result, entry, [], var, pu, s)
    | comp_Undecl (Hermes.ConstDecl(y, i, p) :: res) result entry var label incr pu s =
        let
          val (start, result2, entry2, block, var2, pu2, se2) = comp_Undecl res result entry var label incr pu s
          val (y0, var3) = get_Variable var2 y (Data.TypeS(Data.Public, Data.u64)) var2 false
          val finish = [Data.Assign2S((Data.TypeS(Data.Public, Data.u64), Data.CstS(i, p), Data.VarS(y0, p), p))]
        in
          (start@finish, result2, entry2, block, var3, pu2, se2)
        end
    | comp_Undecl (Hermes.VarDecl(y, t, p) :: res) result entry var label incr pu s =
        let
          val (start, result2, entry2, block, var2, pu2, se2) = comp_Undecl res result entry var label incr pu s
          val t0 = comp_Type t
          val (y0, var3) = get_Variable var2 y t0 var2 false
          val finish = [Data.Assign2S((t0, Data.CstS("0", p), Data.VarS(y0, p), p))]
        in
          (start@finish, result2, entry2, block, var3, pu2, se2)
        end
    | comp_Undecl (Hermes.ArrayDecl(y, t, e, p) :: res) result entry var label i pu s =
        let
          val t0 = comp_Type t
          val size = (get_Size t0) div 8
          val incr = get_Size_Array e
        in
          ...
          then
            let
              val (start, result2, entry2, block, var2, pu2, se2) = comp_Undecl res result entry var label (i+1) pu s
              val (result3, entry3, block2, var3) = array_Undecl_Loop "SMSP" e t0 (start@result2) entry2 size var2 label i p
              val (yt, var4) = get_Variable var3 (y^"T") t0 var3 true
              val (smsp, var5) = get_Variable var4 "SMSP" (Data.TypeS(Data.Public, Data.u64)) var4 false
              val finish = [Data.AssignS((Data.TypeS(Data.Public, Data.u64), Data.CstS("0", p), Data.UpdOp2S("-", Data.VarS(yt, p), Data.SimOp2S(Data.VarS(smsp, p)), p), p))]
              val (ys, var6) = get_Variable var5 (y^"S") t0 var5 true
              val sizeShift = (get_Power (get_Size t0)) - 3
              val (expr, var7) = comp_E (Hermes.Bin(Hermes.ShiftL, e, Hermes.Const(Int.toString(sizeShift), p), p)) "^" (Data.TypeS(Data.Public, Data.u64)) "0" ys p p var6 true
              val var8 = remove_Variable (y^"T") var7
              val var9 = remove_Variable (y^"S") var8 
            in
              (expr@finish, result3, entry3, block@block2, var9, pu2, (se2-(incr*size)))
            end
          else
            let
              val (start, result2, entry2, block, var2, pu2, se2) = comp_Undecl res result entry var label (i+1) pu s
              val (result3, entry3, block2, var3) = array_Undecl_Loop "PMSP" e t0 (start@result2) entry2 size var2 label i p
              val (yt, var4) = get_Variable var3 (y^"T") t0 var3 true
              val (pmsp, var5) = get_Variable var4 "PMSP" (Data.TypeS(Data.Public, Data.u64)) var4 false
              val finish = [Data.AssignS((Data.TypeS(Data.Public, Data.u64), Data.CstS("0", p), Data.UpdOp2S("-", Data.VarS(yt, p), Data.SimOp2S(Data.VarS(pmsp, p)), p), p))]
              val (ys, var6) = get_Variable var5 (y^"S") t0 var5 true
              val sizeShift = (get_Power (get_Size t0)) - 3
              val (expr, var7) = comp_E (Hermes.Bin(Hermes.ShiftL, e, Hermes.Const(Int.toString(sizeShift), p), p)) "^" (Data.TypeS(Data.Public, Data.u64)) "0" ys p p var6 true
              val var8 = remove_Variable (y^"T") var7
              val var9 = remove_Variable (y^"S") var8 
            in
              (expr@finish, result3, entry3, block@block2, var9, (pu2-(incr*size)), se2)
            end
        end
        ...
        in
          ...
          then
            let
              ...
              val size = (get_Power (get_Size ty)) - 3
            in
              if si = 0
              then
                let
                  val start = [Data.AssignS(Data.TypeS(Data.Public, Data.u64), Data.VarS(smsp1, p), Data.UpdOp2S("+", Data.CstS("0", p), Data.Op2S("<<", Data.VarS(atomSize, p), Data.CstS(Int.toString(size), p), p), p), p)]
                in
                ...
                let
                  val start = [Data.AssignS(Data.TypeS(Data.Public, Data.u64), Data.VarS(smsp1, p), Data.UpdOp2S("+", Data.VarS(smsp, p), Data.Op2S("<<", Data.VarS(atomSize, p), Data.CstS(Int.toString(size), p), p), p), p)]
                in
                  comp_Args_Begin pargs var5 (res@[Data.ArgS(Data.TypeS(Data.Public, Data.u64), Data.VarS(atomBegin, p)),
                                                  Data.ArgS(Data.TypeS(Data.Public, Data.u64), Data.VarS(atomSize, p))])
                                        (expr@start) pi pmsp (si+1) smsp1
        ...
        in
          ...
          then
            ...
            in
              ...
              then
                let
                  val start = [Data.AssignS(Data.TypeS(Data.Public, Data.u64), Data.CstS("0", p), Data.UpdOp2S("-", Data.VarS(smsp0, p), Data.Op2S("<<", Data.VarS(atomSize, p), Data.CstS(Int.toString(size), p), p), p), p)]
                in
                  (res2, var7, expr2@start)
                end
              else
                let
                  val start = [Data.AssignS(Data.TypeS(Data.Public, Data.u64), Data.VarS(smsp1, p), Data.UpdOp2S("-", Data.VarS(smsp0, p), Data.Op2S("<<", Data.VarS(atomSize, p), Data.CstS(Int.toString(size), p), p), p), p)]
                in
                  (res2, var7, expr2@start)
                end
            end
  ...
  fun comp_S block res (Hermes.Skip) entry var label incr pu se = (block, entry, res, var, pu, se) (* Skip *)
    ...
    | comp_S block res (Hermes.Block(d, ss, p)) entry var label incr pu se = (* Blocks *)
        let
          val (start, res2, entry2, newblock, var2, pu2, se2) = comp_Decl d res entry var label incr pu se
          val (block2, entry3, stats, var3, pu3, se3) = comp_Block (block@newblock) (res2@start) ss entry2 var2 label incr pu2 se2
          val (finish, res3, entry4, newblock2, var4, pu4, se4) = comp_Undecl d stats entry3 var3 label incr pu3 se3
        in
          (block2@newblock2, entry4, res3@finish, var4, pu4, se4)
        end
  (* Compile Blocks *)
